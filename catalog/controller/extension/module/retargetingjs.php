    ...
    public function sendProduct($product)
    {
        ...
        if (!empty($product))
        {
            ...
            try
            {
                ...
                $setupProduct->setImg(str_replace(' ', '%20', $baseUrl . 'image/' . $product['image']));
                $setupProduct->setPrice(number_format($productPrice, 2, '.', ''));
                $setupProduct->setPromo(number_format($productSpecialPrice, 2, '.', ''));
    ...
    public function saveOrder()
    {
        if ((isset($this->instanceOfThis->session->data['retargeting_save_order']) && !empty($this->instanceOfThis->session->data['retargeting_save_order'])))
        {
            ...
            for ($i = count($orderProductQuery->rows) - 1; $i >= 0; $i--)
            {
                ...
                $product = $this->instanceOfThis->model_catalog_product->getProduct($orderProductQuery->rows[$i]['product_id']);
